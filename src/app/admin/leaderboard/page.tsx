...
export default function AdminLeaderboardPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <span className="px-2 py-1 bg-[#BDDDFC] text-gray-800 rounded-full text-xs font-medium">Unknown</span>
    }
  }
          ...
          <div className="mb-8">
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-2 text-sm md:text-base">Filter leaderboard by time period and distribute prizes to top performers</p>
              </div>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">View All Prize History</span>
                        ...
                        <tr key={user._id} className={`hover:bg-gray-50 ${rank <= 3 ? 'bg-gradient-to-r from-yellow-50 to-transparent' : ''}`}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${rank <= 3 ? prizeInfo.bgGradient + ' text-white' : 'bg-[#BDDDFC] text-gray-700'
                                }`}>
                                {rank}
                              </div>
                              {rank <= 3 && (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {user.prizeTokenStatus?.hasActiveTokens ? (
                              ...
                              <div className="text-xs">
                                <span className="px-2 py-1 bg-[#BDDDFC] text-gray-600 rounded-full text-xs">No Active Tokens</span>
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div key={prize._id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-4 flex-1">
                          <div className={`w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 ${prize.position === 1 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' :
