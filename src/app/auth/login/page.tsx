...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (payment === "success") {
      ...
      toast.success(
        "Payment successful! Please login to access your dashboard.",
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(
        "userToken",
        response.data.userToken || response.data.token,
      ); // Handle both formats
      ...
      if (
        ...
        // Specific handling for deactivation
        setError("Your account has been deactivated.");
        // You could also set a specific state here if you want a custom UI component
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
      ...
      {error && (
        <div
          className={`border px-4 py-3 rounded-lg mb-6 ${error.includes("deactivated") ? "bg-red-50 border-red-200 text-red-700" : "bg-red-50 border-red-200 text-red-700"}`}
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full py-4 px-6 bg-linear-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 font-semibold text-lg disabled:opacity-50 disabled:cursor-not-allowed"
        >
